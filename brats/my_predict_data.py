...
from train_isensee2017 import write_data_to_file
import os
import glob
import tables
import numpy as np
...
def fetch_training_data_files(return_subject_ids=False):
    ...
    for subject_dir in glob.glob(config["to_predict_path"]):
        ...
        subject_files = list()
        for modality in config["training_modalities"] + ["t1"]:
...
def predict_from_file(hdf5_file):
    ...
    for i, modality in enumerate(training_modalities):
        ...
        nib.save(image, "data_{0}.nii.gz".format(modality))
    ...
    if isinstance(prediction_image, list):
        ...
        nib.save(prediction_image, path_name)
...
def my_predict(to_predict_path, predict_out_path, model_file_path):
    ...
    return os.path.join(predict_out_path, "prediction.nii.gz")


def main():
    inn = os.path.join(os.path.dirname(__file__), "data", "brats-to-chaos-learning")
    outt = os.path.abspath("bratschaosprediction")
    model_file_path = config["model_file"]

    for (dirpath, dirnames, names) in walk(inn):
        # filenames = [os.path.join(dirpath, name) for name in names]
        i = 1
        for name in dirnames:
            path = os.path.join(dirpath, name)
            pred_path = my_predict(path, outt, model_file_path)
            new_subject_folder = os.path.join(outt, str(i))
            if not os.path.exists(new_subject_folder):
                os.makedirs(new_subject_folder)
            copyfile(pred_path, os.path.join(new_subject_folder, os.path.basename(pred_path)))
            i = i + 1
